...
namespace SerpoServer.Routes.Admin
{
    public class PageModule : NancyModule
    {
        public PageModule(PageManager pge) : base("/admin/pages")
        {
            ...
            Get("/", x => View["admin/views/pages.html", pge.GetPages((string)Request.Query.domain)]);
            Get("/createoredit", x => View["admin/views/page-editor.html", pge.GetPage((string)Request.Query.domain, Enum.Parse<RequestMethods>(Request.Query.method), (string)Request.Query.path)]);
            Post("/createoredit", x =>
            {
